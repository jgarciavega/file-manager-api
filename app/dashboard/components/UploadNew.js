...
export default function UploadNew() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const usuarios_id = parseInt(localStorage.getItem("user_id"), 10);

      if (!usuarios_id) {
        setErrorMessage("No se encontró ID de usuario en sesión");
        setUploading(false);
        return;
      }
      ...
      formData.append('file', form.file); // clave EXACTA 'file' que espera multer
      formData.append('usuarios_id', usuarios_id);
      formData.append('descripcion', form.observaciones || "");

      // 🔹 Subida con XMLHttpRequest para controlar progreso
      ...
      xhr.addEventListener('load', () => {
        ...
        if (xhr.status === 200 || xhr.status === 201) {
          ...
          if (response.success) {
            setShowModal(true);
            setUploadedFileUrl(response.data?.download_url || "");
          ...
          setErrorMessage(`Error al subir: ${xhr.status} - ${xhr.statusText}`);
        }
      });
      ...
      console.log("Enviando el payload:", { formData, token });

      xhr.open('POST', `${NEXT_PUBLIC_API_URL}/documentos/upload`);
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 bg-gray-50 dark:bg-gray-900`}>
      {/* Usar componente global DashboardHeader (soporta session.user.email o session.email) */}
      <DashboardHeader
        title="Subir Documentos"
        avatarUrl={avatarMap[session?.user?.email || session?.email] || '/login.jpg'}
      />
