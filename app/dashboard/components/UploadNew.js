...
export default function UploadNew() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDocument({ name: "", origin: "", classification: "", jefatura: "", review: "", file: null });
      } else {
  ...
  return (
    <div className={`min-h-screen p-4 transition-all ${darkMode ? "bg-[#0d1b2a] text-gray-100" : "bg-gray-50 text-gray-800"}`}>
      <div className={`flex justify-between items-start mb-6 p-4 rounded-lg ${darkMode ? "bg-[#1a2b3c]" : "bg-white shadow"}`}>
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            <Image src={currentUser.avatar} alt="Avatar" width={50} height={50} className="rounded-full border border-gray-300" />
            <span className="font-medium">{currentUser.name}</span>
          </div>
        </div>
      </div>

      <h1 className="text-4xl font-bold text-center mb-10 text-blue-500">Subir Documento</h1>

      <div className={`p-8 mb-10 rounded-lg shadow-md ${darkMode ? "bg-[#1f2937]" : "bg-white"}`}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          {[
            { label: "Nombre del Documento *", name: "name" },
            ...
            <div key={name}>
              ...
              <input
                ...
                className={`w-full p-2 rounded-md border ${darkMode ? "bg-gray-800 border-gray-600 text-gray-100" : "bg-white border-gray-300 text-gray-800"}`}
              />
            </div>
          ))}
        ...
        <div className="mb-4">
          ...
          <input
            ...
            name="file"
            onChange={handleChange}
            className={`w-full mt-2 rounded border ${darkMode ? "bg-gray-800 border-gray-600 text-gray-100" : "bg-white border-gray-300 text-gray-800"}`}
          />
        </div>
      ...
      <div className="flex justify-end mb-4">
        <div className="relative w-full max-w-sm">
          <input
            ...
            className={`w-full px-4 py-2 pr-10 rounded-md border shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${darkMode ? "bg-gray-800 border-gray-600 text-gray-100 placeholder-gray-400" : "bg-white border-gray-300 text-gray-800 placeholder-gray-500"}`}
          />
          <span className="absolute inset-y-0 right-3 flex items-center pointer-events-none">
            <FontAwesomeIcon icon={faSearch} className={darkMode ? "text-gray-300" : "text-gray-400"} />
          </span>
        </div>
      </div>
      ...
      <div className={`rounded-lg shadow-md ${darkMode ? "bg-[#1e293b]" : "bg-white"}`}>
        <table className="min-w-full text-sm border border-gray-300 dark:border-gray-700">
          <thead>
            <tr className={`text-center font-semibold ${darkMode ? "bg-[#2d3748] text-white" : "bg-gray-200 text-gray-800"}`}>
              {["Folio", "Nombre", "Origen", "Clasificación", "Jefatura", "Reseña", "Fecha", "Responsable"].map((th) => (
                <th key={th} className="p-3 border border-gray-600">{th}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filteredFiles.length === 0 ? (
              ...
              filteredFiles.map((doc) => (
                <tr key={doc.id} className={`text-center ${darkMode ? "even:bg-[#2c3e50] odd:bg-[#1a2634] text-gray-100" : "even:bg-gray-50 odd:bg-white text-gray-800"}`}>
