...
import { faDownload, faUser, faFileAlt, faCalendar, faArrowLeft } from "@fortawesome/free-solid-svg-icons";
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-3xl font-bold text-blue-700 ml-4">Resultados de búsqueda</h1>
      </div>
      {/* Filtros avanzados */}
      <div className="flex flex-wrap gap-4 mb-8 items-center">
        ...
        >
          {CLASIFICACIONES.map(c => (
            <option key={c} value={c}>{c === "Todos" ? "Clasificación" : c}</option>
          ))}
        </select>
        <select
          value={usuario}
        ...
        >
          {usuariosDisponibles.map(u => (
            <option key={u} value={u}>{u === "Todos" ? "Usuario" : u}</option>
          ))}
        </select>
      </div>
      {loading && <p>Buscando documentos...</p>}
      {error && <p className="text-red-500">{error}</p>}
      {!loading && filteredDocs.length === 0 && <p className="text-gray-500">No se encontraron documentos.</p>}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredDocs.map(doc => (
          <div key={doc.id} className="border rounded-xl shadow-lg p-6 bg-white flex flex-col gap-2 hover:shadow-2xl transition">
            <div className="flex items-center gap-2 text-blue-600 font-bold text-lg">
              <FontAwesomeIcon icon={faFileAlt} />
              {doc.nombre || <span className="text-gray-400">Sin nombre</span>}
            </div>
            <div className="text-sm text-gray-600 mb-1">{doc.clasificacion || <span className="text-gray-400">Sin tipo</span>}</div>
            <div className="text-gray-700 text-sm mb-2">{doc.descripcion || <span className="text-gray-400">Sin descripción</span>}</div>
            <div className="flex items-center gap-2 text-xs text-gray-500">
              <FontAwesomeIcon icon={faUser} /> {doc.usuario || "Desconocido"}
              <FontAwesomeIcon icon={faCalendar} className="ml-4" /> {doc.fecha_subida ? new Date(doc.fecha_subida).toLocaleDateString() : "Sin fecha"}
            </div>
            <div className="mt-2 flex gap-4">
              <a href={doc.url || "#"} download className="px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm flex items-center gap-2">
